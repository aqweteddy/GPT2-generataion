...
def embed(documents: dict, ctx_encoder: PreTrainedModel, ctx_tokenizer: BertTokenizerFast) -> dict:
    """Compute the DPR embeddings of document passages"""
    input_ids = ctx_tokenizer(
        documents["title"], documents["text"], truncation=True, max_length=200, padding="max_length", return_tensors="pt"
    )["input_ids"]
...
def main(
    ...
    # ctx_encoder = AutoModel.from_pretrained(rag_example_args.ctx_encoder_model_name).to(device=device)
    # ctx_tokenizer = BertTokenizerFast.from_pretrained(rag_example_args.ctx_encoder_tokenizer_name)
    ...
    passages_path = os.path.join(rag_example_args.output_dir, "wiki_tw_knowledge_dataset")
    ...
    dataset.add_faiss_index("embeddings", custom_index=index, device=1)

    # And save the index
    index_path = os.path.join(rag_example_args.output_dir, "wiki_tw_hnsw_index.faiss")
...
class RagExampleArguments:
    ...
    ctx_encoder_model_name: str = field(
    ...
    ctx_encoder_tokenizer_name: str = field(
