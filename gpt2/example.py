...
def main():
    ...
    for param_index, params in enumerate(all_params):
        ...
        # load the data
        all_train_sentences, all_train_labels, all_test_sentences, all_test_labels = load_dataset(
            params)
        ...
        if params['subsample_test_set'] is None:
            ...
            sample_test_size = min(len(all_test_labels),
                                   params['subsample_test_set'])
            test_sentences, test_labels = random_sampling(
                all_test_sentences, all_test_labels, sample_test_size)
        ...
        if freeze_training_pool:
            ...
            np.random.seed(params['seed'])
        sample_pool_size = min(len(all_train_labels),
                               params['unlabeled_pool_size'])
        train_sentences_pool, train_labels_pool = random_sampling(
            all_train_sentences, all_train_labels, sample_pool_size)

        # sample training examples
        np.random.seed(params['seed'])
        train_sentences, train_labels = random_sampling(
            train_sentences_pool, train_labels_pool, params['num_shots'])

        # get all model responses
        all_responses, all_prompts = get_model_response(
            params, train_sentences, train_labels, test_sentences, return_all_prompts=True)
        ...
        if "gpt2" in params['model']:
            ...
            for entity in cf_tokens:
                prompt = params['prompt_func'](
                    params, train_sentences, train_labels, entity, test_label_option=None)
                cf_prompts.append(prompt)

            all_resp = complete(
                cf_prompts, 1, model=params['model'], num_log_probs=50000)
            for resp in all_resp['choices']:
                log_prob = resp['logprobs']['top_logprobs'][0]
                for token, lp in log_prob.items():
                    cf_probs_dict[token].append(np.exp(lp))  # restore to prob
            ...
            for option in all_options:
                for entity in cf_tokens:
                    prompt = params['prompt_func'](
                        params, train_sentences, train_labels, entity, test_label_option=option)
                    cf_prompts.append(prompt)
            cf_prompts_chunked = list(
                chunks(cf_prompts, chunk_size_helper(params)))
            for chunk_id, prompt_chunk in enumerate(cf_prompts_chunked):
                all_resp = complete(
                    prompt_chunk, 0, model=params['model'], echo=True, num_log_probs=1)
                for resp in all_resp['choices']:
        ...
        for k, v in cf_probs_dict.items():
            # Notice: Min across ensemble of placeholders
            new_cf_dict[k] = np.min(v)
        ...
        for resp in all_responses:
            ...
            calibrate_label_probs = np.matmul(
                W, np.expand_dims(orig_probs_list, axis=-1)) + b
        ...
        if error_frac > 0.01:
            print(f"WARNING: re-encode error frac: {error_frac:.2f}")
    ...
    calibrated_accuracy_list = [
        acc for acc in calibrated_accuracy_list if acc >= 0]
    assert len(orig_accuracy_list) == len(calibrated_accuracy_list)

    orig_accuracy_list = np.reshape(
        orig_accuracy_list, (len(all_lamas), num_seeds))
    calibrated_accuracy_list = np.reshape(
        calibrated_accuracy_list, (len(all_lamas), num_seeds))

    combined_accuracy = np.mean(orig_accuracy_list, axis=0)  # across 41 tasks
    calibrated_combined_accuracy = np.mean(
        calibrated_accuracy_list, axis=0)  # across 41 tasks
